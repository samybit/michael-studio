            ...
            >
              ...
              >
                <div className="w-full h-full overflow-y-auto flex flex-col">
                  <div className="sticky top-0 z-10 flex justify-between items-center p-4 md:p-6 border-b border-border bg-background">
                     <motion.h3 layoutId={`card-title-${project.title.replace(/\s+/g, '-')}`} className="text-2xl md:text-4xl font-black uppercase text-foreground tracking-tighter">
                       {project.title}
                     </motion.h3>
                     <button
                  ...
                  <div className="p-4 md:p-8 flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-1/3 flex flex-col gap-6">
                      <motion.div layoutId={`card-image-container-${project.title.replace(/\s+/g, '-')}`} className="w-full aspect-[4/3] md:aspect-square bg-background border border-border relative overflow-hidden">
                        <SafeImage
                          src={project.imagePath}
                          alt={project.title}
                      ...
                      <motion.p layoutId={`card-desc-${project.title.replace(/\s+/g, '-')}`} className="text-sm md:text-base text-muted-foreground leading-relaxed font-mono">
                        {project.description}
                      </motion.p>
                    </div>
